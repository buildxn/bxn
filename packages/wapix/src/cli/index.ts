...
export async function run(): Promise<void> {
  ...
  if (!command || command === 'start') {
    ...
    console.log('\nUsage: wapix start [options]');
    console.log('\nOptions:');
    console.log('  --port, -p <port>    Port to listen on (default: 3000)');
    console.log('  --routes <path>      Routes directory (auto-detected if not specified)');
    console.log('  --watch              Enable watch mode with Node.js native --watch');
